...
<div style="height: 100%; overflow:auto;">
    ...
    <div>
        <table style="width: 100%" class="table">
            ...
            <c:forEach items="${All}" var="s">
                <tr>
                    <td>${s.ID}</td>
                    <td>
                        Название: ${s.store.name}<br>
                        Телефон: ${s.store.number}<br>
                        Адрес: ${s.store.address}
                    </td>
                    <td>
                        ФИО:${s.customer.fio}<br>
                        Номер телефона: ${s.customer.number}<br>
                        Электронная почта: ${s.customer.mail}
                    </td>
                    <td>${s.purchase_date}</td>
                    <td><a href="UpdatePurchaseServlet?id=${s.ID}">Изменить</a></td>
                    <td><a href="DeletePurchaseServlet?id=${s.ID}">Удалить</a></td>
                </tr>
            </c:forEach>
        </table>
        <button class="btn btn-primary" onclick="window.location.href = 'http://localhost:8082/WallpaperStore_war/AddPurchaseServlet';">Добавьте новую покупку</button>
        <br>
        ...
        <table style="width: 20%; justify-content: center" class="table">
            ...
            <c:forEach items="${getPurchase}" var="c">
                <tr>
                    <td>${c.ID}</td>
                    <td>
                        Название: ${c.store.name}<br>
                        Телефон: ${c.store.number}<br>
                        Адрес: ${c.store.address}
                    </td>
                    <td>
                        ФИО:${c.customer.fio}<br>
                        Номер телефона: ${c.customer.number}<br>
