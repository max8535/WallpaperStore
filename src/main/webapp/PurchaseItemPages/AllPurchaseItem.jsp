    ...
    <div>
        <table style="width: 100%" class="table">
            <tr>
                <th >Покупка</th>
                <th >Товар</th>
                <th >Количество купленного товара,м2</th>
                <th >Цена покупки</th>
                <th ></th>
                <th ></th>
            </tr>
            <c:forEach items="${All}" var="s">
                <tr>
                    <td>
                        Код покупки: ${s.purchase.ID}<br>
                        Дата покупки: ${s.purchase.purchase_date}<br>
                    </td>
                    <td>
                        Код товара: ${s.item.ID}<br>
                        Цена товара: ${s.item.price}<br>
                    </td>
                    <td>
                            ${s.item_amount}
                    </td>
                    <td>${s.price}</td>
                    <td><a href="UpdatePurchaseItemServlet?purchase_id=${s.purchase.ID}&item_id=${s.item.ID}">Изменить</a></td>
                    <td><a href="DeletePurchaseItemServlet?purchase_id=${s.purchase.ID}&item_id=${s.item.ID}">Удалить</a></td>
                </tr>
            </c:forEach>
        </table>
        <button class="btn btn-primary" onclick="window.location.href = 'http://localhost:8082/WallpaperStore_war/AddPurchaseItemServlet';">Добавьте новый купленный товар</button>
        <br>
