    ...
    <div>
        <table style="width: 100%" class="table">
            <tr>
                <th >ID</th>
                <th >Товар</th>
                <th >Количество товара на складе,м2</th>
                <th >Адрес</th>
                <th ></th>
                <th ></th>
            </tr>
            <c:forEach items="${All}" var="s">
                <tr>
                    ...
                    <td>${s.stock_address}</td>
                    <td><a href="UpdateStockServlet?id=${s.ID}">Изменить</a></td>
                    <td><a href="DeleteStockServlet?id=${s.ID}">Удалить</a></td>
                </tr>
            </c:forEach>
        </table>
        <button class="btn btn-primary" onclick="window.location.href = 'http://localhost:8082/WallpaperStore_war/AddStockServlet';">Добавьте новый склад</button>
        <br>
        ...
        <table style="width: 20%; justify-content: center" class="table">
            ...
            <c:forEach items="${getStock}" var="c">
                <tr>
                    <td>${c.ID}</td>
                    <td>
                    ...
                    <td>${c.stock_amount}</td>
                    <td>${c.stock_address}</td>
                </tr>
            </c:forEach>
        </table> -->
    </div>
